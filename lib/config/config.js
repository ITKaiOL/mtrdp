...
(function(app) {
  ...
  app.config.earlyBird = {
    ...
    discountRate: 0.35,
    getDiscount: function(fare, srcStation, destStation) {
      if(app.config.earlyBird.stations.indexOf(destStation) >= 0) {
        return Math.ceil(fare * app.config.earlyBird.discountRate * 10)/10;
      } else {
        return 0;
      }
    }
  ...
  app.config.discount20 = {
    discountRate: 0.2,
    getDiscount: function(fare, srcStation, destStation) {
      return Math.ceil(fare * app.config.discount20.discountRate * 10)/10;
    }
  };
  
  // http://www.mtr.com.hk/en/customer/main/octopus_qr_rebate.html
  // Until Jun 27, 2021
  app.config.discount5 = {
    discountRate: 0.05,
    getDiscount: function(fare, srcStation, destStation) {
      return Math.ceil(fare * app.config.discount5.discountRate * 10)/10;
    }
  };  
