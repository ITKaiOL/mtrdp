(function(app) {
  ...
  function getItinDirect(srcID, destID, options) {
    ...
    for(let i in options) {
      if(options[i] && app.config.hasOwnProperty(i)) {
        discount += app.config[i].getDiscount(baseFare, srcID, destID);
      }
    }
  ...
  function getItinTST(srcID, destID, options) {
    ...
    for(let i in options) {
      if(options[i] && app.config.hasOwnProperty(i)) {
        discount += app.config[i].getDiscount(firstTrip, srcID, destID);
      }
    }
    ...
    if(secondTrip > 0) {
      ...
      for(let i in options) {
        if(options[i] && app.config.hasOwnProperty(i)) {
          discount += app.config[i].getDiscount(secondTripOriginal, srcID, destID);
        }
      }
    } else {
      ...
      for(let i in options) {
        if(options[i] && app.config.hasOwnProperty(i)) {
          minFareDiscount += app.config[i].getDiscount(minFare, srcID, destID);
        }
      }
  ...
  function getOverview(stationID, options) {
    ...
    const fwdOptions = {
      discount5: options.discount5 || false,
      discount20: options.discount20 || false,
      earlyBird: (options.earlyBird === 'fwd' || options.earlyBird === 'rnd') || false
    };
    const bakOptions = {
      discount5: options.discount5 || false,
      discount20: options.discount20 || false,
      earlyBird: (options.earlyBird === 'bak' || options.earlyBird === 'rnd') || false
    };
