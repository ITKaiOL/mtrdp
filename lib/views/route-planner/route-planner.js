(function(app) {
  ...
  function addRoute() {
    ...
    var selectorsDiv;
    var optionEarlyBirdID = 'route-'+routeCounter+'-opt-early';
    var removeButton;
    var resetButton;
    ...
    routeRootElement.appendChild(route.root=app._$.create('div', { className: 'route' }, [
      ...
      route.delete=app._$.create('div', { className: 'remove', style: { display: 'none' } }, [
        removeButton=app._$.create('button', { className: 'remove' }, [
          app._$.matIcon('delete')
        ])
      ]),
      app._$.create('div', { className: 'selectors-options' }, [
        selectorsDiv=app._$.create('div', { className: 'selectors' }),
  ...
  function update() {
    ...
    routes.forEach(function(route, index) {
      ...
      // prepare options
      var stationFrom = route.selectors.from.getValue();
      var stationTo = route.selectors.to.getValue();
      var routeOptions = {
      ...
      if(stationFrom && stationTo) {
        var summary = app.fareCalculator.getSummary(stationFrom, stationTo, routeOptions);
  ...
  function saveRoutes() {
    ...
    var routesData = routes.map(function(route) {
      return {
    ...
    app.autosave.save('route-planner-routes', routesData);    
  }
  
  // load route from storage
  function loadRoutes() {
    var routesData = app.autosave.load('route-planner-routes');
    if(routesData) {
